...
class HMLET_All(nn.Module):
	def __init__(self, 
					config:dict, 
					dataset:BasicDataset):
		super(HMLET_All, self).__init__()
		self.config = config
	...
	def __init_model(self):
		...
		self.gating_net1 = Gating_Net(1, embedding_dim=self.embedding_dim, mlp_dims=self.gating_mlp_dims, dropout_p=self.gating_dropout_prob).to(world.device)
		self.gating_net2 = Gating_Net(2, embedding_dim=self.embedding_dim, mlp_dims=self.gating_mlp_dims, dropout_p=self.gating_dropout_prob).to(world.device)
		self.gating_net3 = Gating_Net(3, embedding_dim=self.embedding_dim, mlp_dims=self.gating_mlp_dims, dropout_p=self.gating_dropout_prob).to(world.device)
		self.gating_net4 = Gating_Net(4, embedding_dim=self.embedding_dim, mlp_dims=self.gating_mlp_dims, dropout_p=self.gating_dropout_prob).to(world.device)
		...
		self.__gating_freeze(self.gating_net3, False)
		self.__gating_freeze(self.gating_net4, False)
	...
	def computer(self, gum_temp, div_noise, hard):     
		...
		# Gating
		stack_embedding_1 = torch.stack([all_emb_lin_1, all_emb_non_1],dim=1)
		concat_embeddings_1 = torch.cat((all_emb_lin_1, all_emb_non_1),-1)
		...
		## layer 2
		all_emb_lin_2 = torch.sparse.mm(g_droped, embedding_1)
		all_emb_non_2 = self.activation_function(torch.sparse.mm(g_droped, embedding_1))

		# Gating
		stack_embedding_2 = torch.stack([all_emb_lin_2, all_emb_non_2],dim=1)
		concat_embeddings_2 = torch.cat((all_emb_lin_2, all_emb_non_2),-1)
		...
		## layer 3
		all_emb_lin_3 = torch.sparse.mm(g_droped, embedding_2)
		all_emb_non_3 = self.activation_function(torch.sparse.mm(g_droped, embedding_2))

		# Gating
		stack_embedding_3 = torch.stack([all_emb_lin_3, all_emb_non_3],dim=1)
		concat_embeddings_3 = torch.cat((all_emb_lin_3, all_emb_non_3),-1)

		gumbel_out_3 = self.gating_net3(concat_embeddings_3, gum_temp, hard, div_noise)
		embedding_3 = self.__choosing_one(stack_embedding_3, gumbel_out_3)

		# Residual embeddings  		
		embs.append(embedding_3)
	
  	
		# layer 4
		all_emb_lin_4 = torch.sparse.mm(g_droped, embedding_3)
		all_emb_non_4 = self.activation_function(torch.sparse.mm(g_droped, embedding_3))

		# Gating
		stack_embedding_4 = torch.stack([all_emb_lin_4, all_emb_non_4],dim=1)
		concat_embeddings_4 = torch.cat((all_emb_lin_4, all_emb_non_4),-1)

		gumbel_out_4 = self.gating_net4(concat_embeddings_4, gum_temp, hard, div_noise)
		embedding_4 = self.__choosing_one(stack_embedding_4, gumbel_out_4)

		# Residual embeddings  		
		embs.append(embedding_4)
	...
	def bpr_loss(self, epoch, users, pos, neg, gum_temp, div_noise, hard):
		...
		if epoch > 300:
			...
			self.__gating_freeze(self.gating_net3, True)
			self.__gating_freeze(self.gating_net4, True)
