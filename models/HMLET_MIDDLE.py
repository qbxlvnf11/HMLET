...
class HMLET_Middle(nn.Module):
	def __init__(self, 
					config:dict, 
					dataset:BasicDataset):
		super(HMLET_Middle, self).__init__()
		self.config = config
	...
	def __init_model(self):
		...
		self.gating_net1 = Gating_Net(2, embedding_dim=self.embedding_dim, mlp_dims=self.gating_mlp_dims, dropout_p=self.gating_dropout_prob).to(world.device)
		self.gating_net2 = Gating_Net(3, embedding_dim=self.embedding_dim, mlp_dims=self.gating_mlp_dims, dropout_p=self.gating_dropout_prob).to(world.device)
	...
	def computer(self, gum_temp, div_noise, hard):     
		...
		# Gating
		stack_embedding_1 = torch.stack([all_emb_lin_2, all_emb_non_1],dim=1)
		concat_embeddings_1 = torch.cat((all_emb_lin_2, all_emb_non_1),-1)
		...
		## layer 3
		all_emb_lin_3 = torch.sparse.mm(g_droped, embedding_1)
		all_emb_non_2 = self.activation_function(torch.sparse.mm(g_droped, embedding_1))

		# Gating
		stack_embedding_2 = torch.stack([all_emb_lin_3, all_emb_non_2],dim=1)
		concat_embeddings_2 = torch.cat((all_emb_lin_3, all_emb_non_2),-1)
		...
		# layer 4
		all_emb_lin_4 = torch.sparse.mm(g_droped, embedding_2)
